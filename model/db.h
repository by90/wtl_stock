...
public:
	//这里获得静态变量的指针
	static std::string *get_default_var()
	{
	...
	{
		*get_default_var() = _default;
		//因此直接略过，用数据库是否正常打开，来判断。
		if (!is_exist(get_default_var()->c_str()))
		{
			if (create_database != nullptr)
				return create_database(get_default_var()->c_str());
			return false;
		}
		return true;
	};
...
public:
	...
	//连接数据库
	DbConnection(const char *filename=DbConnection::get_default())
	{
