...
public:
	void Search(std::function<void(const wchar_t *, int)> func)
	{
		...
		ss.clear();
		ss << L"载入耗时";
		int64_t used = time_used.elapsed_seconds();
		if (used <= 0)
			ss << L"不足1秒!";
		else
			if (used >= 60)
		...
		{
			dbQuote.AdjustPrice(g_stock.Data[i].QuoteSet,g_stock.Data[i].ExRightSet);

		}

		ss << L"复权";
		used = time_used.elapsed_seconds();
		if (used <= 0)
			ss << L"不足1秒!";
		else
			if (used >= 60)
...
class CSearchView : public CDialogImpl<CSearchView>
	, public CCtlColored<CSearchView>
{
public:
	std::thread::id  import_id_;
	enum { IDD = IDD_HOME_BOX};
	SearchViewModel view_model_;
	...
	BEGIN_MSG_MAP(CStockView)
		...
		CHAIN_MSG_MAP(CCtlColored<CSearchView>)
...
//	LRESULT NotifyHandler(int /*idCtrl*/, LPNMHDR /*pnmh*/, BOOL& /*bHandled*/)
	...
	{
		std::thread *t;
		t=new std::thread(&SearchViewModel::Search, &view_model_, [this, t](const wchar_t *info, int now){
			if (info)
			{
