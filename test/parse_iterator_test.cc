...
void int_advance(int * &value)
{
	++value; //前移1个整数
}

TEST_F(ParseIterator, construct_should_work)
{
	//默认构造函数，由编译器生成，不用实现代码
	parse_iterator<int *, char *> iter1;

	////委托构造函数
	//parse_iterator<int *,decltype(advance)> iter2(int_ptr, int_ptr + 4,advance);

	//   //使用{}初始化指针，得到的是nullptr
	//parse_iterator<int *, decltype(advance)> iter3(int_ptr, int_ptr + 4, (int *)int_ptr,advance);
	//++iter3;
}
