...
//要求，迭代器的value_type，兼容int。如果要求完全相同，用is_same,int *也兼容这个
//这里只在返回值部分判断即可，不用在参数部分判断。
template <typename T>
typename std::enable_if<std::is_convertible<typename std::iterator_traits<T>::value_type, int>::value, typename std::iterator_traits<T>::distance_type>::type 
count_vector(T _begin,T _end)
{
